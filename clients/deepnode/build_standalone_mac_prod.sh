#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────
# build_standalone_mac_prod.sh — 构建 Vue 前端 + PyInstaller 打包 deepnode-server（线上环境）
#
# 打包策略:
...
#   - config.yaml            用户可修改配置（使用线上环境 config_prod.yaml）
#   - VERSION                版本号文件
...
#
# 用法:
#   cd clients/deepnode && bash build_standalone_mac_prod.sh
#
# 运行:
...
show_help() {
    cat << 'EOF'
用法:
  cd clients/deepnode && bash build_standalone_mac_prod.sh [选项]

描述:
  构建 Vue 前端 + PyInstaller 打包 deepnode-server 独立运行制品（线上环境）。
  使用 config_prod.yaml 作为配置文件，连接线上 Platform 服务。
...
环境变量:
  TARGET_MACOS_VER    目标 macOS 最低版本号（格式: "主版本_次版本"）。
                      ...
                      示例: TARGET_MACOS_VER=15_0 bash build_standalone_mac_prod.sh
...
示例:
  # 默认构建（自动检测当前平台）
  bash build_standalone_mac_prod.sh

  # 指定目标 macOS 15.0+
  TARGET_MACOS_VER=15_0 bash build_standalone_mac_prod.sh

  # 查看帮助
  bash build_standalone_mac_prod.sh --help
EOF
    exit 0
}
...
echo "═══════════════════════════════════════════════════"
echo "  DeepNode Standalone Builder — PROD (PyInstaller + mlx 隔离)"
...
os.environ['DEEPPOOL_PROFILE'] = 'prod'
...
cp "$LOCALSERVER_DIR/config_prod.yaml" "$ONEDIR/config.yaml"
