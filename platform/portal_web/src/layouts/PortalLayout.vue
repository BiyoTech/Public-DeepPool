<template>
  ...
  <header
    ...
    :class="
      scrolled ? 'bg-white/90 backdrop-blur-lg border-b border-slate-200 shadow-sm' : 'bg-white/80 backdrop-blur-lg'
    "
  >
    <div class="max-w-7xl mx-auto h-full px-6 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center gap-8">
        ...
        >
          ...
          <span
            v-if="item.beta"
            class="ml-1 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold leading-none bg-amber-100 text-amber-700 border border-amber-200 align-top"
            >BETA</span
          >
          <!-- 活跃指示条 -->
          <span v-if="isActive(item.path)" class="absolute bottom-0 left-0 right-0 h-0.5 bg-dp-blue rounded-full" />
        </router-link>
      </nav>
      ...
      <div class="flex items-center gap-3">
        ...
        <template v-if="!authStore.isLoggedIn">
          <router-link to="/login" class="text-sm font-medium text-dp-muted hover:text-dp-title transition-colors">
            {{ $t('nav.login') }}
          </router-link>
          <router-link
            to="/register"
            class="text-sm font-medium px-4 py-2 rounded-full bg-gradient-to-r from-dp-blue to-dp-blue-dark text-white hover:from-dp-blue-dark hover:to-dp-blue-deeper transition-all duration-200 shadow-sm hover:shadow-md"
          >
            {{ $t('nav.register') }}
          </router-link>
        </template>
        <template v-else>
          <div class="relative group">
            <!-- 触发区：用户名 -->
            <button
              class="flex items-center gap-1.5 text-sm font-medium text-dp-muted hover:text-dp-title transition-colors py-1"
            >
            ...
            <!-- Hover 浮层 -->
            <div
              class="invisible group-hover:visible opacity-0 group-hover:opacity-100 absolute right-0 top-full mt-1 w-80 bg-white rounded-2xl shadow-xl border border-slate-100 p-5 transition-all duration-200 z-50"
            >
              <!-- 余额卡片 -->
              ...
              <div class="flex gap-2 mb-3">
                <router-link
                  to="/wallet/recharge"
                  class="flex-1 py-2 text-center rounded-lg bg-gradient-to-r from-dp-blue to-dp-blue-dark text-white text-sm font-medium hover:from-dp-blue-dark hover:to-dp-blue-deeper transition-all shadow-sm hover:shadow-md"
                >
                  {{ $t('wallet.recharge') }}
                </router-link>
                <div
  ...
  >
    <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <div v-if="profileSuccess" class="mb-4 px-4 py-2.5 rounded-lg bg-green-50 text-green-700 text-sm">
        {{ profileSuccess }}
      </div>
      <!-- Error message -->
      <div v-if="profileError" class="mb-4 px-4 py-2.5 rounded-lg bg-red-50 text-red-600 text-sm">
        {{ profileError }}
      </div>
      ...
      <div class="space-y-5">
        ...
        <div class="p-4 rounded-xl border border-slate-100 space-y-3">
          ...
          <button
            ...
            :disabled="profileSaving || !pf.username.trim() || pf.username === authStore.user?.username"
        ...
        <div class="p-4 rounded-xl border border-slate-100 space-y-3">
          ...
          <div class="flex gap-2">
            ...
            <button
              type="button"
              :disabled="emailCooldown > 0 || sendingEmailCode || !pf.newEmail.trim()"
          ...
          <button
            ...
            :disabled="profileSaving || !pf.newEmail.trim() || !pf.emailCode.trim()"
            class="w-full py-2 rounded-lg bg-dp-blue text-white text-sm font-medium hover:bg-dp-blue-dark transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {{ $t('profile.save') }}
          </button>
        ...
        <div class="p-4 rounded-xl border border-slate-100 space-y-3">
          ...
          <input
            v-model="pf.newPassword"
            type="password"
            :placeholder="$t('profile.new_password_placeholder')"
            class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm text-dp-body placeholder:text-dp-placeholder focus:outline-none focus:border-dp-blue focus:ring-2 focus:ring-blue-100"
          />
          <input
            v-model="pf.confirmPassword"
            type="password"
          ...
          <button
            ...
            :disabled="profileSaving || !pf.pwdCode.trim() || !pf.newPassword.trim()"
            class="w-full py-2 rounded-lg bg-dp-blue text-white text-sm font-medium hover:bg-dp-blue-dark transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
            {{ $t('profile.save') }}
          </button>
    ...
    <div class="max-w-7xl mx-auto px-6 py-12">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div>
          ...
          <a
            href="mailto:contact@deeppool.tech"
