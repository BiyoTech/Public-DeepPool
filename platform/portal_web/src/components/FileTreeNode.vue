<template>
  <div>
    ...
    >
      ...
      <!-- File icon -->
      <svg v-else class="w-4 h-4 flex-shrink-0" :class="iconClass" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
      ...
      <!-- Modified time -->
      <span
        v-if="!entry.is_dir && formattedTime"
        class="text-[10px] text-dp-muted/60 whitespace-nowrap"
      >{{ formattedTime }}</span>
    </div>
