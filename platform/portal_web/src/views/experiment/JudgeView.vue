<template>
  <div class="bg-white rounded-xl border border-slate-200 p-6">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <button
        @click="openCreate"
        class="px-4 py-2 rounded-lg bg-dp-blue text-white text-sm font-medium hover:bg-dp-blue-dark transition"
      >
      ...
      >
        <div class="flex items-center justify-between mb-2">
          ...
          <div class="flex items-center gap-2">
            <button
              v-if="judge.status !== 'running'"
              @click="handleRun(judge)"
    ...
    >
      <div
        v-if="showCreate"
        class="fixed inset-y-0 right-0 z-50 w-[560px] max-w-full bg-white shadow-2xl flex flex-col"
      >
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          <h3 class="text-lg font-bold text-dp-title">
            {{ editJudgeId ? $t('experiment.judge.edit_title') : $t('experiment.judge.create_title') }}
          </h3>
        ...
        <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <input
              v-model="form.name"
              type="text"
              :placeholder="$t('experiment.judge.name_placeholder')"
          ...
          <!-- ===== Target Selection ===== -->
          <div>
            <label class="text-xs font-medium text-slate-500 mb-2 block">{{
              $t('experiment.judge.target_label')
            }}</label>
            <p class="text-[11px] text-slate-400 mb-3">{{ $t('experiment.judge.target_hint') }}</p>

            <!-- Step 1: Trace selection (optional) -->
            <div class="mb-3">
              <label class="text-[11px] font-medium text-slate-400 mb-1 block uppercase tracking-wide">{{
                $t('experiment.judge.step_trace')
              }}</label>
              <select
            ...
            <div v-if="form.traceId" class="mb-3 space-y-2">
              <label class="text-[11px] font-medium text-slate-400 mb-1 block uppercase tracking-wide">{{
                $t('experiment.judge.step_filter')
              }}</label>
              ...
              <div v-if="selectedTraceModels.length > 0">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <button
                    v-for="m in selectedTraceModels"
                    :key="m"
                    type="button"
                    class="px-2.5 py-1 rounded-full text-xs border transition"
                    :class="
                      form.filterModels.includes(m)
              ...
              <div v-if="selectedTraceApiKeys.length > 0">
                ...
                <div class="flex flex-wrap gap-1.5">
                  <button
                    v-for="k in selectedTraceApiKeys"
                    :key="k.id"
                    type="button"
                    class="px-2.5 py-1 rounded-full text-xs border transition"
                    :class="
                      form.filterApiKeyIds.includes(k.id)
                        ? 'border-blue-500 bg-blue-50 text-blue-600'
                        : 'border-slate-200 text-slate-500 hover:bg-slate-50'
                    "
            ...
            <div v-if="form.traceId" class="mb-1">
              ...
              <div class="flex items-center gap-2">
                ...
                <span
                  v-if="form.selectedLogIds.length > 0"
                  class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-[11px] font-medium bg-blue-50 text-blue-600"
                >
                  {{
                    $t('experiment.judge.selected_count', {
                      count: form.selectedLogIds.length,
                      total: pickerLogs.length || '?',
                    })
                  }}
            ...
            <div class="relative" ref="judgeModelDropdownRef">
              ...
              >
                ...
                <svg
                  ...
                  :class="{ 'rotate-180': judgeModelDropdownOpen }"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              >
                ...
                >
                  ...
                  <li class="sticky top-0 bg-white px-2 py-1.5 border-b border-slate-100">
                    <input
                      v-model="judgeModelSearch"
                      type="text"
                      :placeholder="$t('experiment.judge.model_search_placeholder')"
                  ...
                  <li
                    v-for="m in filteredJudgeModels"
                    :key="m.id"
                  ...
                  >
                    ...
                    <span
                      :class="vendorTagClass(m.vendor_type)"
                      class="inline-flex shrink-0 items-center rounded px-1.5 py-0.5 text-[10px] font-medium leading-none"
                    >
          ...
          <div>
            <label class="text-xs font-medium text-slate-500 mb-1 block">{{
              $t('experiment.judge.judge_apikey_label')
            }}</label>
            <select
              v-model="form.judgeApiKeyId"
              class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
            >
          ...
          <div>
            <label class="text-xs font-medium text-slate-500 mb-1.5 block">{{
              $t('experiment.judge.scorer_type_label')
            }}</label>
            <div class="flex gap-2">
              <button
                @click="form.scorerType = 'builtin'"
                type="button"
                class="px-3 py-1.5 rounded-lg border text-xs font-medium transition"
                :class="
                  form.scorerType === 'builtin'
                    ? 'border-blue-500 bg-blue-50 text-blue-600'
                    : 'border-slate-200 text-slate-500 hover:bg-slate-50'
                "
              >
              ...
              <button
                @click="form.scorerType = 'custom'"
                type="button"
                class="px-3 py-1.5 rounded-lg border text-xs font-medium transition"
                :class="
                  form.scorerType === 'custom'
          ...
          <div v-if="form.scorerType === 'builtin'">
            <label class="text-xs font-medium text-slate-500 mb-1 block">{{
              $t('experiment.judge.scorer_name_label')
            }}</label>
            <select
              v-model="form.scorerName"
              @change="onScorerChange"
              class="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
            >
              <option value="" disabled>{{ $t('experiment.judge.scorer_select_placeholder') }}</option>
              <option v-for="sc in builtinScorers" :key="sc.name" :value="sc.name">
                {{ sc.display_name }} — {{ sc.description }}
              </option>
            </select>
          </div>
          ...
          <div>
            <label class="text-xs font-medium text-slate-500 mb-1 block">{{
              $t('experiment.judge.prompt_label')
            }}</label>
            <textarea
              v-model="form.promptTemplate"
              rows="8"
              :placeholder="$t('experiment.judge.prompt_placeholder')"
          ...
          <!-- Error -->
          <div v-if="createError" class="px-3 py-2 rounded-lg bg-rose-50 border border-rose-200 text-xs text-rose-600">
            {{ createError }}
          </div>
        </div>
        ...
        <div class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 shrink-0">
          ...
          <button
            @click="handleCreate"
            :disabled="creating"
            class="px-4 py-2 rounded-lg bg-dp-blue text-white text-sm font-medium hover:bg-dp-blue-dark transition disabled:opacity-50"
          >
      ...
      <div v-if="runsJudge" class="fixed inset-y-0 right-0 z-50 w-[700px] max-w-full bg-white shadow-2xl flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto px-6 py-4">
          ...
          <div v-if="!activeRunView">
            ...
            <div v-else-if="runs.length === 0" class="text-center py-12 text-dp-muted text-sm">
              {{ $t('experiment.judge.no_runs') }}
            </div>
              ...
              >
                <div class="flex items-center justify-between mb-2">
                  ...
                  <span
                    ...
                    :class="
                      run.pass_rate >= 80 ? 'text-emerald-600' : run.pass_rate >= 50 ? 'text-amber-600' : 'text-red-500'
                    "
                  >
                ...
                <div v-if="run.total_count > 0" class="mt-2 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    :style="{
                      width: `${Math.min(100, ((run.completed_count + run.failed_count) / run.total_count) * 100)}%`,
                    }"
                  ></div>
                </div>
              </div>
          ...
          <div v-else>
            ...
            <div v-else-if="results.length === 0" class="text-center py-12 text-dp-muted text-sm">
              {{ $t('experiment.judge.no_results') }}
            </div>
            <table v-else class="w-full text-sm">
              <thead>
                <tr class="border-b border-slate-200">
                  <th class="py-2 px-2 text-left text-xs font-medium text-slate-500">
                    {{ $t('experiment.judge.col_log_id') }}
                  </th>
                  <th class="py-2 px-2 text-left text-xs font-medium text-slate-500">
                    {{ $t('experiment.judge.col_passed') }}
                  </th>
                  <th class="py-2 px-2 text-left text-xs font-medium text-slate-500">
                    {{ $t('experiment.judge.col_reason') }}
                  </th>
                  <th class="py-2 px-2 text-left text-xs font-medium text-slate-500">
                    {{ $t('experiment.judge.col_duration') }}
                  </th>
                  <th class="py-2 px-2 text-left text-xs font-medium text-slate-500">
                    {{ $t('experiment.judge.col_status') }}
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="r in results" :key="r.id" class="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td class="py-2 px-2">
                    <span
                      v-if="r.success"
                      class="px-2 py-0.5 rounded-full text-[10px] font-medium"
                      :class="r.passed ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'"
                    >
                  ...
                  <td class="py-2 px-2 text-xs max-w-[280px]" :class="r.success ? 'text-slate-600' : 'text-red-600'">
                    <div
                      v-if="expandedResultId === r.id"
                      class="whitespace-pre-wrap break-all cursor-pointer"
                      @click="expandedResultId = null"
                    >
      ...
      <div v-if="showLogPicker" class="fixed inset-0 z-[60] flex items-center justify-center p-6">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[80vh] flex flex-col overflow-hidden">
            ...
            <table v-else class="w-full text-sm">
              <thead class="bg-slate-50 text-dp-muted text-xs uppercase tracking-wider sticky top-0 z-10">
                <tr>
                  <th class="px-4 py-3 text-left font-medium w-10">
                    <input
                      type="checkbox"
                      :checked="pickerAllSelected"
                      :indeterminate="pickerSomeSelected"
                      @change="toggleAllPickerLogs"
              ...
              <tbody class="divide-y divide-slate-100">
                ...
                >
                  ...
                  <td class="px-4 py-3 text-right font-mono text-xs text-dp-title">
          ...
          <div class="flex items-center justify-between px-6 py-3 border-t border-slate-100 bg-slate-50/50 shrink-0">
            ...
            <div class="flex gap-2">
              <button
                @click="cancelLogPicker"
                class="px-4 py-1.5 rounded-lg border border-slate-200 text-xs text-slate-500 hover:bg-white transition"
              >
                {{ $t('experiment.judge.cancel') }}
              </button>
              <button
                @click="confirmLogPicker"
...
const selectedJudgeModelOption = computed(
  () => availableModels.value.find((m) => m.id === form.value.judgeModel) || null,
)
...
const pickerAllSelected = computed(
  () => pickerLogs.value.length > 0 && pickerSelectedIds.value.length === pickerLogs.value.length,
)
...
const targetSummary = computed(() => {
  ...
  if (f.selectedLogIds.length > 0) {
    ...
    if (f.filterModels.length > 0)
      parts.push(t('experiment.judge.summary_models', { names: f.filterModels.join(', ') }))
...
async function fetchData() {
  ...
  try {
    const [judgesRes, tracesRes, keysRes, scorersRes] = await Promise.allSettled([
      ...
      listTraces(),
      listAPIKeys(),
      listBuiltinScorers(locale.value),
    ])
...
async function handleCreate() {
  ...
  if (!f.judgeModel.trim()) {
    createError.value = 'Judge model is required'
    return
  }
  if (!f.judgeApiKeyId) {
    createError.value = 'Judge API Key is required'
    return
  }
  if (!f.promptTemplate.trim()) {
    createError.value = 'Prompt is required'
    return
  }
  if (f.scorerType === 'builtin' && !f.scorerName) {
    createError.value = 'Select a builtin scorer'
    return
  }
...
async function handleRun(judge: JudgeDTO) {
  ...
  try {
    await runJudge(judge.id)
    await refreshJudges()
  } catch (err: any) {
    alert(err?.response?.data?.message || err?.message || 'Run failed')
  }
}
...
async function loadMoreResults() {
  ...
  try {
    ...
    resultsLoadingMore.value = false
  }
}
