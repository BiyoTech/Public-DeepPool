    ...
    <template v-else>
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div class="rounded-lg border border-slate-100 bg-slate-50/50 p-4">
            ...
            <div class="mt-1.5 flex items-center gap-1.5 text-xs">
              <span
                class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600 font-medium"
              >
          ...
          <!-- Models card -->
          <div class="rounded-lg border border-slate-100 bg-slate-50/50 p-4">
            <p class="text-xs font-medium text-dp-muted uppercase tracking-wide mb-2">
              {{ $t('experiment.overview.models') }}
            </p>
            <div class="flex items-baseline gap-2">
          ...
          <div class="rounded-lg border border-slate-100 bg-slate-50/50 p-4">
            ...
            <div class="mt-1.5 text-[11px] text-dp-muted">
              {{ $t('experiment.overview.tracking') }}: <b class="text-dp-title">{{ stats.tracedApiKeys }}</b> keys,
      ...
      <div class="bg-white rounded-xl border border-slate-200 p-6">
        <h2 class="text-base font-semibold text-dp-title mb-4 flex items-center gap-2">
          ...
          <span class="ml-2 text-xs font-normal text-dp-muted">{{
            $t('experiment.overview.judge_total', { count: judges.length })
          }}</span>
        </h2>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-slate-200">
                <th class="py-2.5 px-3 text-left text-xs font-medium text-dp-muted">
                  {{ $t('experiment.overview.col_name') }}
                </th>
                <th class="py-2.5 px-3 text-left text-xs font-medium text-dp-muted">
                  {{ $t('experiment.overview.col_model') }}
                </th>
                <th class="py-2.5 px-3 text-left text-xs font-medium text-dp-muted">
                  {{ $t('experiment.overview.col_scorer') }}
                </th>
                <th class="py-2.5 px-3 text-left text-xs font-medium text-dp-muted">
                  {{ $t('experiment.overview.col_target') }}
                </th>
                <th class="py-2.5 px-3 text-center text-xs font-medium text-dp-muted">
                  {{ $t('experiment.overview.col_status') }}
                </th>
                <th class="py-2.5 px-3 text-right text-xs font-medium text-dp-muted">
                  {{ $t('experiment.overview.col_pass_rate') }}
                </th>
              </tr>
            </thead>
            <tbody>
      ...
      <div class="bg-white rounded-xl border border-slate-200 p-6">
        <h2 class="text-base font-semibold text-dp-title mb-2 flex items-center gap-2">
          ...
          <span class="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-400 font-normal">{{
            $t('experiment.coming_soon')
          }}</span>
        </h2>
